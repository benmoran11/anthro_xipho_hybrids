...
month_colors <- c('#01161E','#4D91A8','#D295BF')
names(month_colors) = c("Feb", "May", "Sep")
...
# Loading same dataset as in other analyses, but not filtering out September

water_chemistry_Calnali_censored_raw <- read.csv("waterchem_with_averagedsonde_labmetals_data.csv") %>%
  ...
  mutate(day = sapply(Date, function(x) strsplit(x, split = "-")[[1]][1]),
         ...
         month = ifelse(month == "Jun", "May", month)) %>% # Collapsing June into May since measurements were contiguous (late May/early June)
...
nonmetal_pca_Calnali <- fviz_pca_biplot(wc.pca.nonmetals.Calnali, axes = c(1,2), repel = T, label = "var", alpha.var = 0.20, mean.point = FALSE, col.var = "black", habillage = pca_nonmetals_Calnali$month, title = NULL)+
  ...
  scale_color_manual(values = month_colors, labels = c("Feb","May-Jun","Sep")) +
...
ggsave("nonmetal_pca_Calnali_monthly.pdf",
...
metal_pca_Calnali
ggsave("metal_pca_Calnali_monthly.pdf",
...
Calnali_monthly_PCAs
ggsave("Calnali_monthly_PCAs.pdf",
       Calnali_monthly_PCAs, width = 6.5, height = 8)
ggsave("Calnali_monthly_PCAs.png",
       Calnali_monthly_PCAs, width = 6.5, height = 8)


# Now Fig S9

water_chemistry_alltime <- read.csv("waterchem_with_averagedsonde_labmetals_data.csv") %>%
...
alltime_monthly_PCAs
ggsave("alltime_monthly_PCAs.pdf",
       alltime_monthly_PCAs, width = 6.5, height = 8)
ggsave("alltime_monthly_PCAs.png",
       alltime_monthly_PCAs, width = 6.5, height = 8)
...
ggsave("Calnali_monthly_line_plots.png", 
...
write.csv(sample_sizes_alltime, "waterchem_samplesizes_alltime.csv", sep = ",")
